...
# with open(r"E:\work\RPA\中间文件\flow\06510004-GXAP-20220223-0022\flow.json", 'r', encoding='utf-8') as f:
#     data = json.load(f)
# home_dir = r'E:\work\RPA\中间文件\港口一期'
# with open(r'C:/RPA/test.json', 'w', encoding='utf-8') as f:
#    json.dump(data, f)
...
def get_excel_data(path2, path3, flow):
    flow['data']['baseInfo']['公司'].strip()
    flow['data']['baseInfo']['部门'].strip()
    flow['data']['baseInfo']['付款金额'].strip()
    flow['data']['advice'] = []
    if not all(
            [flow['data']['baseInfo']['公司'], flow['data']['baseInfo']['部门'],
             flow['data']['baseInfo']['付款金额'], flow['data']['baseInfo']['费用项目']]):
    ...
    exapproval = []
    re_int = re.compile('\d+')
    price = flow['data']['baseInfo']['付款金额'].replace(',', '')
