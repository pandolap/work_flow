...
def compareApproval(flow):
    ...
    # flow["data"]["advice"].insert(0, '机器人试运行，审批意见：')
...
def offline_payment_review(flow):
    add_approval = []
    wrong_opinion = []
    base_info = flow.get('data').get('baseInfo')
    home_dir = flow['config'].get('home_dir')
    No = base_info.get('单据编号')
    payment_bank = base_info.get('付款银行', '')
    ...
    flow['data']['verifyResult'] += ';' + '；'.join(wrong_opinion) + '；' + '；'.join(pay_comment)
